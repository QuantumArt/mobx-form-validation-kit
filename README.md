...
| `onChangeValidValue: UpdateValidValueHandler<TEntity>` | передает последние валидное значение |
| `callSetterOnInitialize</b>: boolean` | вызвать 'onChangeValidValue' при создании `FormControl` |
| `callSetterOnReinitialize: boolean` | вызывать `onChangeValidValue` при каждом изменении результата функции-геттера из первого аргумента |
...
И проверив. Самый верхний можно узнать валидности формы.
...
| `notEmptyOrSpacesValidator` | проверят, что значение не равно null, а для строк проверят строку на пустоту или не состоит из одних пробелов. |
| `notContainSpacesValidator` | проверят, что строка не содержит пробелов. |
...
•	Что валидаци отработают не только когда меняется значение, но и когда поменялась observable переменная внутри валидации.
...
•	Как делать сложную валидацию с запросами на сервер и приэтом жто врё рано будет отслеживаемой
И еще кучу всего, что позволят из коробки mobx-form-validation-kit.
